...
namespace MarbleWebProject.ViewComponents
{
    ...
    public class MainBannerViewComponent : ViewComponent
    {
        ...
        public MainBannerViewComponent(IMemoryCache cache)
        {
            _cache = cache;
        }
        public async Task<IViewComponentResult> InvokeAsync(List<AllBannerResponse> models)
        {
            ...
            string Key = "MainBanner" + AppConfig.CMSService.CustomName + AppConfig.CMSService.LanguageCode ;
            string returnHtml = "";
            ...
            if (!cacheHelper.IfCache(Key))
            {
                using (var cms = new CmsClient())
                {
                    ...
                    var contentRequest = new BannerAllRequest { LanguageCode = loginResponse.LanguageCode,Type="MAINSLIDER" };
